  ...
  return (
    <div className="min-h-screen transition-colors duration-300 bg-white dark:bg-[#121212] font-sans text-slate-900 dark:text-white flex flex-col items-center py-12 px-6">
      ...
      <button 
        ...
        className="absolute p-2 transition border rounded-full shadow-sm top-8 right-8 border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800"
      >
      ...
      <header className="flex flex-col items-center mb-8 text-center">
        ...
        <h1 className="mb-1 text-3xl font-bold">Neto Monteiro</h1>
        ...
        <div className="flex gap-2">
          <span className="flex items-center gap-1 px-4 py-1 text-xs font-medium rounded-full bg-slate-100 dark:bg-slate-800">
            <Globe size={12} /> Web
          </span>
          <span className="flex items-center gap-1 px-4 py-1 text-xs font-medium rounded-full bg-slate-100 dark:bg-slate-800">
             UI/UX 
      ...
      <main className="w-full max-w-md mb-12 space-y-4">
        <LinkCard 
...
const LinkCard = ({ icon, title, url }) => (
  ...
  >
    <div className="mr-4 text-slate-600 dark:text-slate-400">
      {icon}
    </div>
...
const SocialIcon = ({ icon, url }) => (
  <a 
    ...
    target="_blank" 
    rel="noopener noreferrer"
    className="transition-colors text-slate-400 hover:text-slate-900 dark:hover:text-white"
  >
    {icon}
  </a>
);
